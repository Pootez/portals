...
class Portals {
    ...
    project(x, y, lines) {
        ...
        let Aa = new Line(this.a.x1, this.a.y1, this.a.x1 + cos(AaAng) * 20, this.a.y1 + sin(AaAng) * 20)
        let Ab = new Line(this.a.x2, this.a.y2, this.a.x2 + cos(AbAng) * 20, this.a.y2 + sin(AbAng) * 20)
        let Ba = new Line(this.b.x1, this.b.y1, this.b.x1 + cos(BaAng) * 20, this.b.y1 + sin(BaAng) * 20)
        let Bb = new Line(this.b.x2, this.b.y2, this.b.x2 + cos(BbAng) * 20, this.b.y2 + sin(BbAng) * 20)
        ...
        for (const line of l) {
            ...
            let aLeft = (ray.x) * -(line.y1 - this.a.y1) + (ray.y) * (line.x1 - this.a.x1) > 0
            let bLeft = (ray.x) * -(line.y2 - this.a.y1) + (ray.y) * (line.x2 - this.a.x1) > 0
        ...
        for (const line of temp) {
            ...
            let leftright = (line.x1 - x) * -(line.y2 - y) + (line.y1 - y) * (line.x2 - x) < 0
        ...
        for (const line of temp) {
            ...
            let aLeft = (ray.x) * -(line.y1 - this.b.y1) + (ray.y) * (line.x1 - this.b.x1) > 0
            let bLeft = (ray.x) * -(line.y2 - this.b.y1) + (ray.y) * (line.x2 - this.b.x1) > 0
        ...
        for (const line of l) {
            let ray = this.b.getVector()
            let posLeft = (ray.x) * -(xyAB.y - this.b.y1) + (ray.y) * (xyAB.x - this.b.x1) <= 0
            let aLeft = (ray.x) * -(line.y1 - this.b.y1) + (ray.y) * (line.x1 - this.b.x1) > 0
            let bLeft = (ray.x) * -(line.y2 - this.b.y1) + (ray.y) * (line.x2 - this.b.x1) > 0
        ...
        for (const line of temp) {
            ...
            // x = (line.x1 - x) y = (line.y1 - y)
            let aleftb = (BaV.x) * -(BbV.y) + (BaV.y) * (BbV.x) < 0
            let leftright = (line.x1 - xyAB.x) * -(line.y2 - xyAB.y) + (line.y1 - xyAB.y) * (line.x2 - xyAB.x) < 0
            let rightofa = (BaV.x) * -(line.y1 - xyAB.y) + (BaV.y) * (line.x1 - xyAB.x) > 0
            ...
            } else if ((!aleftb && rightofa && leftofb)||(aleftb && !rightofa && !leftofb)) {
                Al.push(line)
            }
        }
        ...
        for (const line of l) {
            let ray = this.a.getVector()
            let posLeft = (ray.x) * -(xyBA.y - this.a.y1) + (ray.y) * (xyBA.x - this.a.x1) <= 0
            let aLeft = (ray.x) * -(line.y1 - this.a.y1) + (ray.y) * (line.x1 - this.a.x1) > 0
            let bLeft = (ray.x) * -(line.y2 - this.a.y1) + (ray.y) * (line.x2 - this.a.x1) > 0
        ...
        for (const line of temp) {
            ...
            // x = (line.x1 - x) y = (line.y1 - y)
            let aleftb = (AaV.x) * -(AbV.y) + (AaV.y) * (AbV.x) < 0
            let leftright = (line.x1 - xyBA.x) * -(line.y2 - xyBA.y) + (line.y1 - xyBA.y) * (line.x2 - xyBA.x) < 0
            let rightofa = (AaV.x) * -(line.y1 - xyBA.y) + (AaV.y) * (line.x1 - xyBA.x) > 0
